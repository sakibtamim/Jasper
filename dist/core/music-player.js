import { joinVoiceChannel, createAudioPlayer, AudioPlayerStatus, NoSubscriberBehavior, } from "@discordjs/voice";
import { ActionRowBuilder } from "discord.js";
import logger from "./logger.js";
import workerPool from "./worker-pool.js";
import { validateInteraction, setVoiceStatus, getChannelName, formatDuration, } from "./utils/voice-utils.js";
import { getQueue, setQueue, deleteQueue, getAllQueues, cleanupWorkerOldQueues, clearAllQueues, } from "./audio/queue-manager.js";
import { fetchVideoData, fetchPlaylistData, isUrl, } from "./audio/stream-handler.js";
...
async function assignWorker(interaction, voiceChannel) {
    ...
    if (worker.role === "worker") {
        await interaction.channel
            ?.send(`🐾 **Jasper** is busy, summoning **${worker.name}** to handle the beats!`)
            .catch(() => { });
    }
    return worker;
}
async function validateAndCleanupQueue(interaction, voiceChannelId) {
    ...
    if (queue.voiceChannelId !== voiceChannelId) {
        ...
        logger.info(`User switched channels from ${oldChannelName} (${queue.voiceChannelId}) to ${newChannelName} (${voiceChannelId}), cleaning up old queue`);
        ...
        if (queue.songs.length > 0) {
            ...
            if (queue.textChannel) {
                try {
                    ...
                    const channelName = channel && 'name' in channel ? channel.name : "the voice channel";
                    queue.textChannel
...
async function enqueue(interaction, query) {
    ...
    try {
        ...
        if (queue.songs.length === 1 && !queue.nowPlaying) {
            ...
            await interaction.editReply(`✅ **${queue.worker.name}** added to queue in **#${channelName}**: [${track.title}](${track.url})`);
        }
        else {
            await interaction.editReply(`✅ **${queue.worker.name}** queued in **#${channelName}**: [${track.title}](${track.url})`);
        }
    }
    catch (error) {
        logger.error(error);
...
async function enqueuePlaylist(interaction, url) {
    ...
    try {
        ...
        await interaction.editReply(`✅ **Added ${songsToAdd.length} songs** from playlist: **${data.title || "YouTube Playlist"}**${truncatedMsg}`);
    }
    catch (error) {
