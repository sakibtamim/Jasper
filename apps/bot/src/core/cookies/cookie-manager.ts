...
export class CookieManager {
     ...
     */
    async getBestCookiePath(): Promise<{ path: string; cookieId: number } | null> {
        try {
    ...
    async cleanupCookieFile(filePath: string): Promise<void> {
        try {
            ...
            logger.warn(`[CookieManager] Failed to cleanup cookie file ${filePath}: ${error}`);
        }
    }
            ...
            try {
                ...
                if (cookieId) {
                    ...
                    if (isAuthError) {
                        ...
                        logger.warn(`[CookieManager] Cookie ${cookieId} failed auth check. Retrying... (Attempt ${attempt + 1}/${effectiveRetries})`);
                    } else {
                        logger.warn(`[CookieManager] Operation failed with cookie ${cookieId}: ${error.message}`);
                    }
                }
