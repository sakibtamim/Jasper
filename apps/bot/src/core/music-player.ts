...
async function enqueuePlaylist(interaction: ChatInputCommandInteraction, url: string, options: EnqueueOptions = {}): Promise<void> {
  const voiceChannel = await validateInteraction(interaction);
  if (!voiceChannel) return;
  ...
  try {
    ...
    let truncated = false;
    const limit = options.limit || 50; // Default to 50 if not specified, or use provided limit
    if (data.title && data.title.startsWith("Mix -") && entries.length > limit) {
      entries = entries.slice(0, limit);
      truncated = true;
      logger.info(
        `Truncated autogenerated playlist '${data.title}' to ${limit} songs.`
      );
    }
    ...
    const songsToAdd: Song[] = entries.map((entry: any) => ({
