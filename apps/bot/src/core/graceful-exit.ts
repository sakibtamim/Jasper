 ...
 */
export async function handleGracefulExit(
  reason: string,
  error?: Error,
): Promise<void> {
  if (isShuttingDown) {
    logger.warn(
      `[gracefulexit] Shutdown already in progress (Reason: ${reason})`,
    );
    return;
  }
  ...
  if (error) {
    logger.error(
      `[gracefulexit] Error details: ${error.stack || error.message}`,
    );
  }
  ...
  await Promise.all(
    workers.map(async (worker) => {
      try {
        ...
        const msg = err instanceof Error ? err.message : String(err);
        logger.error(
          `[gracefulexit] Error destroying client for ${worker.name}: ${msg}`,
        );
      }
    }),
  );
