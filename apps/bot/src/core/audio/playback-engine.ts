...
export async function handleAutoplay(
  ...
  try {
    ...
    const individualVideos = searchResult.videos.filter(
      (video: ytSearch.VideoSearchResult) => {
    ...
    // Check audio cache if enabled (for YouTube only)
      ...
      if (storage) {
        ...
        if (cachedStream) {
          ...
          audioSource = await storage.cacheAudioStream(song.url, videoId, [
            song.title,
          ]);
...
export async function handleRadio(queue: Queue): Promise<void> {
  try {
    ...
    if (queue.textChannel && "send" in queue.textChannel) {
      queue.textChannel.send(
        "❌ An error occurred while trying to play radio.",
      );
    }
    queue.isRadio = false;
  }
