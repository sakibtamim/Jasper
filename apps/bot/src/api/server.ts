...
export async function startServer() {
  ...
  try {
    ...
    logger.info(
      `[webui] Backend API server running at http://localhost:${PORT}`,
    );
    if (isDevelopment) {
      logger.info(
        `[webui] React Dashboard: http://localhost:5173 (Vite dev server)`,
      );
      logger.info(
      ...
      logger.info(
        `[webui] Legacy UI: http://localhost:${PORT}/legacy/index.html`,
      );
    }
