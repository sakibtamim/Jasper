...
const devtoolsRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.delete("/api/devtools/cache/:query", async (request, reply) => {
    ...
    try {
      ...
      logger.error(
        `[devtools] Failed to delete cache entry ${decodedQuery}: ${error}`,
      );
  ...
  fastify.delete(
    "/api/devtools/cache/audio/:videoId",
    async (request, reply) => {
      ...
      try {
        ...
        logger.error(
          `[devtools] Failed to delete audio cache entry ${videoId}: ${error}`,
        );
        return reply
          .status(500)
          .send({ error: "Failed to delete audio cache entry" });
      }
    },
  );

  fastify.post(
    "/api/devtools/cache/audio/:videoId/regenerate-thumbnail",
    async (request, reply) => {
      const { videoId } = request.params as { videoId: string };
      ...
      try {
        ...
        logger.error(
          `[devtools] Failed to regenerate thumbnail for ${videoId}: ${error}`,
        );
        return reply
          .status(500)
          .send({ error: "Failed to regenerate thumbnail" });
      }
    },
  );
  ...
  fastify.delete("/api/devtools/stats/songs", async (request, reply) => {
    ...
    try {
      ...
      logger.error(
        `[devtools] Failed to delete plays for song ${url}: ${error}`,
      );
      return reply
        .status(500)
        .send({ error: "Failed to delete plays for song" });
    }
  });
  ...
  fastify.delete("/api/devtools/stats/users/:id", async (request, reply) => {
    ...
    try {
      ...
      logger.error(
        `[devtools] Failed to delete plays for user ${id}: ${error}`,
      );
      return reply
        .status(500)
        .send({ error: "Failed to delete plays for user" });
    }
  });
  ...
  fastify.delete("/api/devtools/stats/channels/:id", async (request, reply) => {
    ...
    try {
      ...
      logger.error(
        `[devtools] Failed to delete plays for channel ${id}: ${error}`,
      );
      return reply
        .status(500)
        .send({ error: "Failed to delete plays for channel" });
    }
  });
  ...
  fastify.delete("/api/devtools/stats/bots/:name", async (request, reply) => {
    ...
    try {
      ...
      logger.error(
        `[devtools] Failed to delete plays for bot ${name}: ${error}`,
      );
      return reply
        .status(500)
        .send({ error: "Failed to delete plays for bot" });
    }
  });
  ...
  fastify.post<{ Params: { id: string }; Body: { enabled: boolean } }>(
    "/api/devtools/plugins/:id/toggle",
    async (request, reply) => {
      ...
      if (typeof enabled !== "boolean") {
        reply
          .status(400)
          .send({ error: "Invalid body: enabled must be a boolean" });
        return;
      }
      ...
      try {
        ...
        if (result.success) {
          ...
          reply
            .status(400)
            .send({ error: result.message || "Failed to toggle plugin" });
        }
      } catch (error) {
  ...
  fastify.post<{ Params: { id: string }; Body: { enabled: boolean } }>(
    "/api/devtools/cookies/:id/toggle",
    async (request, reply) => {
      const { id } = request.params;
      const { enabled } = request.body;
