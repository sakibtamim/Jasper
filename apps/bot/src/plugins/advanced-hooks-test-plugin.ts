...
const AdvancedHooksTestPlugin: Plugin = {
  ...
  onLoad: async (context: PluginContext) => {
    ...
    context.on("SERVER_READY", (data: any) => {
      ...
      logger.info(
        "[AdvancedHooksTestPlugin] Registered /api/test-plugin route.",
      );
    });
    ...
    context.on("WORKER_ASSIGNED", (data: any) => {
      ...
      logger.info(
        `[AdvancedHooksTestPlugin] WORKER_ASSIGNED: ${worker.name} -> Guild ${guildId} / Channel ${voiceChannelId}`,
      );
    });
    ...
    context.on("VOICE_STATE_UPDATE", (data: any) => {
      ...
      logger.info(
        `[AdvancedHooksTestPlugin] VOICE_STATE_UPDATE: ${oldState?.member?.user.username} moved from ${oldState?.channelId} to ${newState?.channelId}`,
      );
    });
  },
