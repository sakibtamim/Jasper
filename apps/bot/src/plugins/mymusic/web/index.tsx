        ...
        try {
            const res = await fetch('/api/plugins/mymusic/profiles', {
                ...
                body: JSON.stringify({
                    name: newName,
                    content: newContent,
                    user_agent: newUserAgent || undefined
                })
            });
            ...
            <div className="space-y-6">
                ...
                <Card className="w-full">
                    <div className="flex justify-between items-center mb-6">
                        <div>
                            <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Music Profiles</h3>
                        ...
                        <>
                            {isAdding && (
                                <div className="mb-6 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 space-y-4">
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">User Agent (Recommended)</label>
                                        <Input
                                            value={newUserAgent}
                                            onChange={(e) => setNewUserAgent(e.target.value)}
                                            placeholder="Mozilla/5.0..."
                                        />
                                        <p className="text-xs text-gray-500 mt-1">Found in your browser's network tab. Helps avoid detection.</p>
                                    </div>
                                    <div>
                                            ...
                                            <tr key={profile.id}>
                                                <td className="font-medium text-gray-900 dark:text-white">
                                                    ...
                                                    <div className="text-xs text-gray-400 font-mono mt-0.5">
                                                        {profile.userAgent ? `UA: ${profile.userAgent.substring(0, 30)}...` : 'No UA'}
                                                    </div>
                                                </td>
                                                <td>
                                                    <Badge variant={profile.healthStatus === 'valid' ? 'success' : 'warning'}>
                                                        {profile.healthStatus || profile.status}
                                                    </Badge>
                                                </td>
                                                <td>
