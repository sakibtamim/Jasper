...
export const SoundboardPage = () => {
    ...
    try {
      if (editingSound) {
        ...
        const res = await fetch(
          `/api/plugins/soundboard/sounds/${editingSound.id}`,
          {
            method: "PATCH",
  ...
  const handleClearData = async () => {
    if (
      !confirm(
        "Are you sure you want to clear ALL soundboard data? This cannot be undone!",
      )
    )
      return;
  ...
  const handlePreview = (soundId: string) => {
    ...
    if (sound) {
      setPreviewAudio(
        `/api/plugins/soundboard/storage/${sound.fileUri.split("/").pop()}`,
      );
    }
  };
