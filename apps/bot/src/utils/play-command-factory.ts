...
function isAudioFile(filename: string, contentType: string | null): boolean {
  ...
  const hasValidContentType = contentType
    ? SUPPORTED_AUDIO_CONTENT_TYPES.some((type) => contentType.startsWith(type))
    : false;
  ...
  return {
    data: new SlashCommandBuilder()
      ...
      .addAttachmentOption((option) =>
        option
          .setName("file")
          .setDescription(
            "Audio file to play (mp3, ogg, wav, flac, m4a, webm, opus)",
          )
          .setRequired(false),
      ),
    ...
    async autocomplete(interaction: AutocompleteInteraction) {
      ...
      try {
        ...
        logger.error(
          `[commands] Autocomplete failed for query "${focusedValue}": ${error instanceof Error ? error.message : String(error)}`,
    ...
    async execute(interaction: ChatInputCommandInteraction) {
      ...
      if (!attachment && !query) {
        await interaction.reply({
          content:
            "❌ Please provide either a search query/URL or an audio file attachment.",
          ephemeral: true,
        });
        return;
      }
      ...
      if (attachment) {
        ...
        logger.info(
          `[commands] /${name}: Playing from attachment: ${attachment.name}`,
        );
      } else if (query) {
        ...
        if (
          ...
          try {
            const urlToParse = query.startsWith("http")
              ? query
              : `https://${query}`;
            ...
            playlistWarning =
              "⚠️ **I'm only playing the first song.**\nIf you want to queue the whole playlist, please use `/playlist`!";
          } catch (error) {
            logger.warn(
              `Failed to parse/clean URL in /${name} command: ${error}`,
            );
          }
        }
      }
      ...
      if (!query) {
        logger.error(
          `[commands] /${name}: Reached enqueue with empty query after validation.`,
          { suppressOnWebUI: true },
        );
        await interaction.reply({
          content:
            "❌ Something went wrong determining what to play. Please try again.",
          ephemeral: true,
        });
        return;
      }
