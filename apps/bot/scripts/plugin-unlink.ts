...
if (!fs.existsSync(linkPath)) {
    console.error(`Error: Plugin "${pluginId}" is not installed (path not found: ${linkPath})`);
    process.exit(1);
}
...
try {
    ...
    if (!stats.isSymbolicLink()) {
        console.error(`Error: "${pluginId}" is a real directory, not a symlink. Use "rm -rf" if you really want to delete it.`);
        process.exit(1);
    }
