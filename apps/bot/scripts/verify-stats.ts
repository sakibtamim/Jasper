...
async function verify() {
  try {
    ...
    if (topSongs[0].songTitle !== "Song A")
      throw new Error("Expected Song A to be top");
    if (topSongs[0].playCount !== 2)
      throw new Error("Expected Song A to have 2 plays");

    if (topUsers.length !== 2) throw new Error("Expected 2 top users");
    if (topUsers[0].userId !== "user1")
      throw new Error("Expected user1 to be top");

    if (globalStats.totalPlays !== 3) throw new Error("Expected 3 total plays");
    if (globalStats.totalDuration !== 420)
      throw new Error("Expected 420 total duration");
