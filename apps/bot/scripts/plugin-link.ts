...
if (!fs.existsSync(manifestPath)) {
    console.error(`Error: Target directory does not contain a jasper-plugin.json file.`);
    process.exit(1);
}
...
try {
    ...
    if (fs.existsSync(linkPath)) {
        ...
        if (stats.isSymbolicLink()) {
            ...
            console.error(`Error: A directory already exists at ${linkPath}. Please remove it manually.`);
            process.exit(1);
        }
    }
