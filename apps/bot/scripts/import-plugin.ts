...
async function main() {
    ...
    // 1. Validate Manifest
    const manifestEntry = zipEntries.find(e => e.entryName === 'jasper-plugin.json');
    if (!manifestEntry) {
        console.error('❌ Invalid plugin: jasper-plugin.json not found in zip root.');
        process.exit(1);
    }
    ...
    if (isProd) {
        ...
        if (isSourceZip) {
            console.warn('⚠️  Warning: Importing source code into PROD environment. This plugin may not run without compilation.');
        }
    } else {
        // Dev Mode -> src/plugins
        targetDir = path.join(PLUGINS_SRC_DIR, pluginId);
        if (!isSourceZip && hasIndexJs) {
            console.warn('⚠️  Warning: Importing compiled code into DEV environment. You will not be able to edit source.');
        }
    }
