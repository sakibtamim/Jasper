    ...
    const handleAddCookie = async () => {
        ...
        try {
            const response = await fetch('/api/devtools/cookies', {
                ...
                body: JSON.stringify({ name: newCookieName, content: newCookieContent }),
            });
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h2 className="text-2xl font-bold tracking-tight text-gray-900 dark:text-white">YouTube Cookies</h2>
                    <p className="text-muted-foreground text-gray-500 dark:text-gray-400 mt-1">
                        Manage cookies for yt-dlp to bypass "Sign in to confirm you’re not a bot" errors.
                    </p>
                </div>
                <Button onClick={() => {
            ...
            {mounted && isAddDialogOpen && createPortal(
                <div
                    className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm"
                    style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 9999, backgroundColor: 'rgba(0,0,0,0.6)' }}
                >
                    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-lg overflow-hidden border border-gray-200 dark:border-gray-700">
                        <div className="p-6 border-b border-gray-200 dark:border-gray-700">
                            <h3 className="text-xl font-semibold text-gray-900 dark:text-white">Add New Cookie</h3>
                        ...
                        <div className="p-6 space-y-4">
                            ...
                            <div className="space-y-2">
                                ...
                                <p className="text-xs text-gray-500 dark:text-gray-400 flex items-center gap-1">
                                    <span className="inline-block w-4 h-4 bg-blue-100 text-blue-600 rounded-full text-center leading-4 font-bold text-[10px]">i</span>
                        ...
                        <div className="p-6 bg-gray-50 dark:bg-gray-900/50 border-t border-gray-200 dark:border-gray-700 flex justify-end gap-3">
                            ...
                            <Button onClick={handleAddCookie} disabled={!newCookieName || !newCookieContent}>
                                Save Cookie
                            </Button>
                        </div>
                    </div>
            ...
            <Card className="overflow-hidden border border-gray-200 dark:border-gray-700 shadow-sm">
                <div className="p-6 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800">
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Stored Cookies</h3>
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                    ...
                    <table className="w-full text-sm text-left">
                        <thead className="text-xs text-gray-500 uppercase bg-gray-50 dark:bg-gray-900/50 dark:text-gray-400 border-b border-gray-200 dark:border-gray-700">
                            <tr>
                                ...
                                <th scope="col" className="px-6 py-3 font-medium">Success Rate</th>
                                <th scope="col" className="px-6 py-3 font-medium">Usage</th>
                                <th scope="col" className="px-6 py-3 font-medium">Last Used</th>
                                ...
                                <tr>
                                    <td colSpan={6} className="px-6 py-12 text-center text-muted-foreground">
                                        <div className="flex flex-col items-center gap-2">
                                            <div className="w-12 h-12 bg-gray-100 dark:bg-gray-800 rounded-full flex items-center justify-center text-2xl">🍪</div>
                                            <p className="font-medium text-gray-900 dark:text-white">No cookies found</p>
                                            <p className="text-sm text-gray-500">Add a cookie to improve playback reliability.</p>
                                        </div>
                                    </td>
                                </tr>
                            ) : (
                                cookies.map((cookie) => (
                                    <tr key={cookie.id} className="bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors group">
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-3">
                                                <button
                                                    onClick={() => handleToggleCookie(cookie.id, !cookie.isActive)}
                                                ...
                                                <span className={`text-xs font-medium ${cookie.isActive ? 'text-green-600 dark:text-green-400' : 'text-gray-500'}`}>
                                        ...
                                        <td className="px-6 py-4">
                                            <Badge variant={
                                                cookie.failureCount === 0 && cookie.successCount > 0 ? 'success' :
                                                    cookie.failureCount > cookie.successCount ? 'error' : 'default'
                                            } className="font-mono">
                                                {getSuccessRate(cookie.successCount, cookie.failureCount)}
                                            </Badge>
                                        </td>
                                        <td className="px-6 py-4">
                                        ...
                                        <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-400 whitespace-nowrap">
                                            {cookie.lastUsed ? formatDistanceToNow(new Date(cookie.lastUsed), { addSuffix: true }) : 'Never'}
                                        </td>
