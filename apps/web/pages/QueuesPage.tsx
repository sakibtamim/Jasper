                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                        {queues.map((queue) => (
                            <QueueCard
                                ...
                                queue={queue}
                                isExpanded={expandedSongs.has(
                                    `${queue.guildId}-${queue.voiceChannelId}`,
                                )}
                                onToggle={() =>
                                    toggleSongExpansion(`${queue.guildId}-${queue.voiceChannelId}`)
                                }
                        ...
                        <div className="flex items-center justify-center gap-4">
                            ...
                            <div className="text-sm text-gray-600 dark:text-gray-400">
                                ...
                                <span className="text-xs ml-2">
                                    ({pagination.totalQueues} total queues)
                                </span>
                            </div>
...
function QueueCard({ queue, isExpanded, onToggle, formatDuration, formatEta }: QueueCardProps) {
    ...
    return (
        <div
            id={`queue-${queue.guildId}-${queue.voiceChannelId}`}
            className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-4 hover:border-brand-primary/30 transition-colors shadow-sm scroll-mt-24"
        >
                            ...
                            {isExpanded ? (
                                ...
                                <>
                                    ...
                                    {Math.min(
                                        ...
                                        filteredSongs.length - maxInitialSongs,
                                    )}{' '}
                                    More Songs
                                    {canExpand &&
                                        ` (${filteredSongs.length - maxExpandedSongs} more not shown)`}
                                </>
                            )}
                        </button>
                    )}
