...
function WorkerCard({ worker }: { worker: Worker }) {
    ...
    return (
        <div className={`bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg border-l-4 ${borderColor} transition-all hover:scale-[1.02] relative overflow-hidden group dark:border-t-0 dark:border-r-0 dark:border-b-0`}>
            {/* Music Note BG */}
            ...
            <div className="flex items-start gap-4 mb-4 relative z-10">
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-start">
                        <div>
                            <h3 className="text-xl font-bold text-gray-900 dark:text-white truncate">{worker.name}</h3>
                            <p className="text-sm text-gray-500 dark:text-gray-400 capitalize">{worker.role}</p>
                        </div>
                        <span className={`text-xs font-medium px-2 py-1 rounded-full bg-gray-100 dark:bg-gray-700 ${statusColor}`}>
                            {statusText}
                        </span>
                    </div>
                </div>
            ...
            <div className="space-y-3 relative z-10">
                ...
                {worker.guildId && (
                    <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300">
                        {worker.guildIconUrl ? (
                            <img src={worker.guildIconUrl} className="w-4 h-4 rounded-full object-cover" alt="" />
                        ) : (
                    ...
                    <div className="mt-3 pt-3 border-t border-gray-100 dark:border-gray-700">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div className="min-w-0">
                                <p className="text-sm font-medium text-gray-900 dark:text-white truncate" title={worker.nowPlaying.title}>
