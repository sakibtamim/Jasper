        ...
        for (const channelId of affectedChannelIds) {
            ...
            try {
                ...
                if (nonBotMembers.size === 0) {
                    if (queue.player.state.status === AudioPlayerStatus.Playing) {
                        ...
                        if (queue.textChannel && 'send' in queue.textChannel) {
                            queue.textChannel.send("⏸️ **Auto-Paused** because the voice channel is empty.").catch(error => logger.warn(`[voice-state] Failed to send auto-pause message: ${error}`));
                        }
                    }
                }
                // Case 2: Someone joined and we are auto-paused -> Auto Resume
                    ...
                    if (queue.textChannel && 'send' in queue.textChannel) {
                        queue.textChannel.send("▶️ **Auto-Resumed** because a user joined.").catch(error => logger.warn(`[voice-state] Failed to send auto-resume message: ${error}`));
                    }
                }
            } catch (error) {
                logger.error(`[voice-state] Error handling voice state update for channel ${channelId}: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
    }
};
